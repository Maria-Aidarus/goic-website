...
function mo_sps_show_test_connection(type) {
    ...
    var testWindow = window.open(appConfig.test_url + "&type=" + type + (db_id != '' ? "&id=" + db_id : ''), "TEST Connection", "scrollbars=1 width=800, height=600");
    var timer = setInterval(checkChild, 500);
    function checkChild() {
        if (testWindow.closed) {
            clearInterval(timer);
            location.href = appConfig.admin_url + '?page=mo_sps&tab=app_config';
        }
    }
}
