...
class Azure
{
    ...
    private function setEndpoints()
    {
        ...
        if("success" == get_option("test_connection_manual_status")){
            $api_endpoint = 'graph.microsoft.com';
        }
        else {
    ...
    public function mo_sps_process_tokens_for_auto_connection() {
        ...
        if($response['status']) {
            ...
            if($connector == 'personal') {
                ...
                $jwt_object = json_decode(base64_decode(str_replace('_', '/', str_replace('-','+',explode('.', $this->access_token)[1]))), true);
                update_option("mo_sps_user_upn_automatic_connection",$jwt_object['upn']);
                $name = $jwt_object['given_name']." ".$jwt_object['family_name'];
